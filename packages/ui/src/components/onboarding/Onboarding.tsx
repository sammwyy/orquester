...
export const Onboarding: React.FC = () => {
  ...
  const [startWorkerOnLogin, setStartWorkerOnLogin] = useState(false);
  ...
  const install = async () => {
    ...
    try {
      ...
      await workerManager.configure({ startWorkerOnLogin, remoteAccess, port: Number(localPort), username: localUsername, password: localPassword, serveWeb, workspacesDir });
      await updateAppConfig({ runInBackground, startWorkerOnLogin, setupComplete: true, localWorkerInstalled: true });
    ...
    <Modal open onClose={() => undefined} className="max-w-xl" backdropDraggable>
      <div className="w-full p-6">
        ...
        {step === "local" && <div className="mt-5 space-y-4">
          ...
          <div className="rounded-xl border border-neutral-800 bg-neutral-900/60 p-3"><div className="flex items-start gap-3"><Settings2 size={18} className="mt-0.5 text-neutral-400" /><div className="flex-1"><p className="text-sm text-neutral-200">Start worker when I sign in</p><p className="mt-0.5 text-xs text-neutral-500">The worker starts independently of the desktop app after you sign in.</p></div><Switch checked={startWorkerOnLogin} onChange={setStartWorkerOnLogin} /></div></div>
